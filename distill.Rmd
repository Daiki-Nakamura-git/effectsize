...
2つ目は、非心ｔ分布を用いる方法である。これは、母効果量 $\delta$ の推定値 $\hat\delta$ が非心度 $ncp=\hat\delta\sqrt{\frac{mn}{m+n}}$ 、自由度 $(m+n-2)$ の非心ｔ分布に従うことを利用する方法である。非心ｔ分布の詳細については、[@haebara2014]などを参照されたい。
...
library(effectsize)
ds <- cohens_d(A, B)  # Cohen's d_s
ncp <- ds$Cohens_d*sqrt(m*n/(m+n)) # 非心度パラメータ
df <- m+n-2 # 自由度
library(MBESS)
lambda.ci <- conf.limits.nct(ncp=ncp, df=df, conf.level = .95) # 非心t分布の95%CI
...
対応のある2群の平均値差の効果量の信頼区間の計算についても、独立した2群の場合と同様の3種類の方法が使用できる。ここでは、非心ｔ分布に基づく方法のみ紹介する。母効果量 $\delta_p$ の推定値 $d_z$ が非心度 $ncp=\hat\delta_p\sqrt{N}$ 、自由度 $(N-1)$ の非心ｔ分布に従うことを利用すればよい。
...
ncp <- dz$Cohens_d*sqrt(n) # 非心度パラメータ
df <- n-1 # 自由度
library(MBESS)
lambda.ci <- conf.limits.nct(ncp=ncp, df=df, conf.level = .95) # 非心t分布の95%CI
lambda.ci$Lower.Limit/sqrt(n-1)  # 95%信頼区間の下限値
lambda.ci$Upper.Limit/sqrt(n-1)  # 95%信頼区間の上限値
```
...
ところで、Cohenの規準が多くの分野の実際の効果量分布と適合しているのであれば問題はないかもしれないが、実際には適合していないことが多い。例えば、ソフトウェア工学分野の92の実験をレビューした @kampenes2007 は、効果量の分布が全体的にCohenの規準よりも大きいことを指摘している。医学分野においても、効果量の分布がCohenの規準よりも大きいことが指摘されいている[@cherubini2021]。このように分野における効果量分布がCohenの規準と適合していないのであれば、やはりCohenの規準の使用を正当化する理由はないだろう。
